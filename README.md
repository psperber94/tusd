...
[latest release](https://github.com/psperber94/tusd/releases/latest).
...
Please consult the [online documentation](https://godoc.org/github.com/psperber94/tusd)
...
[`filestore`](https://godoc.org/github.com/psperber94/tusd/filestore) which will save every upload
...
is as simple as implementing the [`tusd.DataStore`](https://godoc.org/github.com/psperber94/tusd/#DataStore)
interface and using the new struct in the [configuration object](https://godoc.org/github.com/psperber94/tusd/#Config).
...
* [**s3store**](https://godoc.org/github.com/psperber94/tusd/s3store): A storage backend using AWS S3
* [**filestore**](https://godoc.org/github.com/psperber94/tusd/filestore): A storage backend using the local file system
* [**gcsstore**](https://godoc.org/github.com/psperber94/tusd/gcsstore): A storage backend using Google cloud storage
* [**memorylocker**](https://godoc.org/github.com/psperber94/tusd/memorylocker): An in-memory locker for handling concurrent uploads
* [**consullocker**](https://godoc.org/github.com/psperber94/tusd/consullocker): A locker using the distributed Consul service
...
The tusd binary, once executed, listens on the provided port for only non-encrypted HTTP requests and *does not accept* HTTPS connections. This decision has been made to limit the functionality inside this repository which has to be developed, tested and maintained. If you want to send requests to tusd in a secure fashion - what we absolutely encourage, we recommend you to utilize a reverse proxy in front of tusd which accepts incoming HTTPS connections and forwards them to tusd using plain HTTP. More information about this topic, including sample configurations for Nginx and Apache, can be found in [issue #86](https://github.com/psperber94/tusd/issues/86#issuecomment-269569077) and in the [Apache example configuration](/docs/apache2.conf).
